...
### Validation Flow
1. **Setup**: Validate an explicitly selected existing Project, then create a validation workflow with a fixed sequence (Text -> Image -> Video) and the current `auto_publish: true`, `provider: youtube` contract.
...
- **Existing Project Required**: Requires `AI_VIDEO_OS_PRODUCTION_E2E_PROJECT_ID` to identify a Project already present in the target database. The runner does not create Projects.
- **Mandatory Private Status**: The script constructs the YouTube provider with `privacy_status="private"`, independent of a more permissive general application setting, and the runner verifies the returned metadata.
- **Secret Redaction**: Unexpected exception messages and presigned URLs are not copied into the validation report.
- **Current Production Dependencies**: Uses `S3ObjectStorage`, `PromptBuilder()`, repository-backed `PublishingQueueService`, the encrypted YouTube credential resolver, and the current `AutomaticPublishingCoordinator` constructor.

## Usage
To run the production E2E validation:
```bash
export AI_VIDEO_OS_RUN_PRODUCTION_E2E=true
export AI_VIDEO_OS_PRODUCTION_E2E_PROJECT_ID=<existing-project-uuid>
python scripts/production_e2e_validation.py
```

Keep the opt-in disabled during ordinary development and CI. Setting a Project ID alone never
